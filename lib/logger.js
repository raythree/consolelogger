...
function getMTime() {
  try {
    ...
    return fs.statSync(config.configFile).mtime;
  }
  catch (e) {
    return null
  }
...
function watchConfig() {
  if (process.env.NODE_ENV === 'production') {
    config.watchTimer = setTimeout(watchConfig, config.watchInterval*1000)
    var mtime =  getMTime()
    if (!mtime) return

    if (config.lastMTime && config.lastMTime !== mtime) {
      config.lastMTime = mtime
      configFromFile(config.configFile)
    }
  }
  else {
...
function resetConfig() {
  ...
  if (config.configFile && config.watch) fs.unwatchFile(config.configFile)
  if (config.watchTimer) { clearTimeout(config.watchTimer); config.watchTimer = null }
...
if (fileExists(defaultLogConfig)) {
  ...
  if (opts) {
    config.configFile = defaultLogConfig
    configure(opts)
  }
}
